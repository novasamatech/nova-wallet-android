...
class BannerPagerScrollController(private val context: Context, private val callback: ScrollCallback) {
    ...
    fun onInterceptTouchEvent(ev: MotionEvent): Boolean {
        return when (ev.action) {
            MotionEvent.ACTION_DOWN, MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> true

            MotionEvent.ACTION_MOVE -> scrollTracking.eventDx().absoluteValue > 100 // To support only horizontal swipe
