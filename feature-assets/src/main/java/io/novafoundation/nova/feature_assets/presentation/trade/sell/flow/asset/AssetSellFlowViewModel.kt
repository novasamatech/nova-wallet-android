...
class AssetSellFlowViewModel(
    ...
    override fun assetClicked(asset: Chain.Asset) {
        validate(asset) {
            router.openSellProviders(asset.chainId, asset.id)
        }
    }
    ...
    override fun tokenClicked(tokenGroup: TokenGroupUi) {
        when (val type = tokenGroup.groupType) {
            ...
            TokenGroupUi.GroupType.Group -> router.openSellNetworks(NetworkFlowPayload(tokenGroup.tokenSymbol))
        }
    }
}
