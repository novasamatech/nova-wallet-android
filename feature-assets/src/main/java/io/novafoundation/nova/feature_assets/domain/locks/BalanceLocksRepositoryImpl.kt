...
class BalanceLocksRepositoryImpl(
    ...
    override suspend fun observeBalanceLocks(chain: Chain, chainAsset: Chain.Asset): Flow<List<BalanceLock>> {
        ...
        return locksDao.observeBalanceLocks(metaAccount.id, chain.id, chainAsset.id).map { list ->
    ...
    override suspend fun observeLocksForMetaAccount(metaAccount: MetaAccount): Flow<List<BalanceLock>> {
        return combine(locksDao.observeLocksForMetaAccount(metaAccount.id), chainRegistry.chainsById) { locks, chains ->
            locks.map {
