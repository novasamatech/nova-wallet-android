...
val AddProxyAccount_53_54 = object : Migration(53, 54) {
    ...
    override fun migrate(database: SupportSQLiteDatabase) {
        ...
        database.execSQL(
            """
            ALTER TABLE 'meta_accounts' ADD COLUMN `status` TEXT NOT NULL DEFAULT ${MetaAccountLocal.Status.ACTIVE}, 
            ADD COLUMN `parentMetaId` INTEGER, 
            FOREIGN KEY(`parentMetaId`) REFERENCES `meta_accounts`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE
            """.trimMargin()
        )
