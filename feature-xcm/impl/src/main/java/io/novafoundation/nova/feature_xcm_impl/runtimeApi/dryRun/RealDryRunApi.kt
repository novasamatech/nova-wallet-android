...
class RealDryRunApi @Inject constructor(
    ...
    override suspend fun dryRunCall(
        ...
        resultXcmsVersion: XcmVersion,
        chainId: ChainId
    ): Result<ScaleResult<CallDryRunEffects, DryRunEffectsResultErr>> {
        return multiChainRuntimeCallsApi.forChain(chainId).dryRunCall(originCaller, call, resultXcmsVersion)
    }
        ...
        return runCatching {
            call(
                ...
                arguments = mapOf(
                    ...
                    "call" to call,
                    "result_xcms_version" to resultXcmsVersion.version.toBigInteger()
                ),
                returnBinding = {
                    runtime.provideContext {
                        ScaleResult.bind(
